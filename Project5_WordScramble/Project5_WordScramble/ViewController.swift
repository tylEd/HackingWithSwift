...
class ViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.leftBarButtonItem = UIBarButtonItem(title: "New", style: .plain, target: self, action: #selector(newGamePrompt))
    ...
    @objc func newGamePrompt() {
        let ac = UIAlertController(title: "Start a New Game", message: nil, preferredStyle: .alert)
        ac.addAction(UIAlertAction(title: "No", style: .default))
        ac.addAction(UIAlertAction(title: "Yes", style: .default) { _ in self.startGame() })
        ...
        if isPossible(word: lowerAnswer) {
            if isOriginal(word: lowerAnswer) {
                if isReal(word: lowerAnswer) {
                    usedWords.insert(lowerAnswer, at: 0) //NOTE: Solution to bug fix
